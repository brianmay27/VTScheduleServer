...
public class CASManager {
    ...
    public static final String USER_AGENTS				= "Mozilla/5.0 (Windows NT 6.3; WOW64; rv:25.0) Gecko/20100101 Firefox/25.0";
	...
	private boolean login() throws LoginException {
		try {
			...
			conn.setInstanceFollowRedirects(false);
			conn.setRequestProperty("Cookie", arrayToString(authCookies));
			conn.connect();
			...
			conn.setRequestMethod("GET");
			conn.setRequestProperty("User-Agent", USER_AGENTS);
			conn.setRequestProperty("Cookie", arrayToString(hokiespaCookies));
			conn.connect();
			hokiespaCookies = getCookies(conn);
			url = conn.getHeaderField("Location");
			conn.disconnect();
